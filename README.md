...
본 프로젝트는 AI 도구를 적극 활용하여 개발되었습니다.
...
| **평가 기준 참조**      | 18건 (REQUIREMENTS.md 섹션 명시적 참조) |

### 비판적 검증 주요 사례

AI 제안을 그대로 수용하지 않고, 요구사항과 아키텍처 일관성을 기준으로 검토 후 수정했습니다.

| AI 제안                     | 실제 채택           | 수정 이유                            |
| --------------------------- | ------------------- | ------------------------------------ |
| Context API + useMemo       | Zustand             | 선택적 구독으로 리렌더링 최적화      |
| 배열 기반 필터 상태         | Set 자료구조        | O(1) 조회, 중복 방지                 |
| Next.js 유지                | Vite로 전환         | CSR 전용 대시보드에 SSR 불필요       |
| 수동 useMemo/useCallback    | React Compiler 도입 | 자동 메모이제이션, 코드 간결화       |
| 잘못된 status에 기본값 대체 | 필터링으로 제외     | 데이터 무결성 우선                   |
| 단일 Y축 차트               | 듀얼 Y축            | 노출수/클릭수 스케일 차이로 정보손실 |

### 협업 방법론: 하네스 프로그래밍

**REQUIREMENTS.md**의 평가 기준을 AI에게 명시적으로 제공하고, 프롬프트에서 섹션 번호를 참조하여 방향을 제어하는 방식을 적용했습니다.

```
# 프롬프트 예시
REQUIREMENTS.md 3.3 캠페인 관리 테이블 구현:
- 평가 포인트: 일괄 상태 변경
- 기존 Feature-based 아키텍처 유지
```

상세한 활용 내역은 **[AI_USAGE.md](./AI_USAGE.md)** 를 참조해주세요.
