...
export const dailyStatApi = {
  ...
  getByCampaignId: async (campaignId: string): Promise<DailyStat[]> => {
    const raw = await api.get<unknown[]>(`/daily_stats?campaignId=${campaignId}`);
    return parseArray(dailyStatSchema, raw);
  },
  getByDateRange: async (startDate: string, endDate: string): Promise<DailyStat[]> => {
    const raw = await api.get<unknown[]>(`/daily_stats?date_gte=${startDate}&date_lte=${endDate}`);
    return parseArray(dailyStatSchema, raw);
  },
};
